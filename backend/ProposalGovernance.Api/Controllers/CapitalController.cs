...
namespace ProposalGovernance.Api.Controllers
{
    ...
    public class CapitalController : ControllerBase
    {
        ...
        public async Task<IActionResult> RequestDrawdown([FromBody] DrawdownRequest request)
        {
            ...
            if (role != UserRoles.Admin && role != UserRoles.Investor && proposal.SubmitterId != userId)
                return Forbid();
            ...
            // Notify all project investors about drawdown
            var investments = await _investmentRepository.GetByProposalIdAsync(proposal.Id);
            foreach (var inv in investments)
            {
                var invNotification = new Notification
                {
                    UserId = inv.InvestorId,
                    Title = "Disbursement from Funded Project",
                    Message = $"A drawdown of {request.Amount:C} was executed for your invested proposal '{proposal.Title}'. Description: {tx.Description}."
                };
                await _notificationRepository.AddAsync(invNotification);

                await _hubContext.Clients.Group($"User_{inv.InvestorId}").SendAsync("ReceiveNotification", new
                {
                    id = invNotification.Id,
                    title = invNotification.Title,
                    message = invNotification.Message,
